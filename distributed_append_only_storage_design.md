...
2. **强一致性**：同一 Chunk 的所有分片在相同 offset 处数据完全一致
...
| 组件 | 职责 |
|------|------|
| **MetaServer** | 维护全局元数据，包括 Chunk 路由信息（分片位置、状态）和文件到 Chunk 的映射信息（目录树、命名空间）；管理 Chunk 生命周期；调度分片放置与修复任务 |
| **ChunkServer** | 存储 Chunk 的分片；执行追加写入和读取；维护本地数据完整性（CRC 校验）；执行分片修复；向 MetaServer 周期性上报心跳 |
...
Chunk 是系统的**逻辑数据存储单元**，是用户和 Client 视角下的基本操作对象。每个 Chunk 由 MetaServer 在创建时分配一个全局唯一的 64 位 ID（`chunk_id`），具有以下特征：
...
- **冗余存储**：Chunk 的数据通过冗余编码策略被拆分为多个**分片（Shard）**，分布在不同的 ChunkServer 上（详见 3.2 节）

Chunk 是一个**逻辑概念**——用户和 Client 只需关心 Chunk 整体的读写，无需感知底层分片的存在。系统内部负责将对逻辑 Chunk 的操作转化为对各个分片的并行操作。

```
逻辑视角（Client / 用户）                 物理视角（ChunkServer）

                                      ┌── ChunkServer A ───┐
                                      │  Shard 0 (data)     │
┌──────────────────┐                  ├── ChunkServer B ───┤
│                  │                  │  Shard 1 (data)     │
│   Chunk 1001     │   EC(4,2) 编码   ├── ChunkServer C ───┤
│  （逻辑数据单元）  │ ──────────────► │  Shard 2 (data)     │
│   64MB 用户数据   │                  ├── ChunkServer D ───┤
│                  │                  │  Shard 3 (data)     │
└──────────────────┘                  ├── ChunkServer E ───┤
                                      │  Shard 4 (code)     │
                                      ├── ChunkServer F ───┤
...
Chunk 分片（Shard）是 Chunk 数据在**物理存储层面的实际单元**。每个分片存储在单个 ChunkServer 的单块磁盘上，是 ChunkServer 直接管理和操作的对象。分片通过 `chunk_id` + `shard_index` 唯一标识。

根据冗余编码策略的不同，分片的类型和含义也不同：

| 编码类型 | 分片构成 | 分片与原始数据的关系 |
|----------|----------|---------------------|
| **Replica** | N 个 replica shard | 每个 replica shard 都是 Chunk 数据的完整拷贝 |
| **EC(k,m)** | k 个 data shard + m 个 code shard | data shard 共同构成 Chunk 的完整数据，code shard 用于容错恢复 |
| **LRC** | data shard + code shard + local shard | 在 EC 基础上增加 local shard（本地校验分片），加速单分片修复 |

Chunk 与 Shard 的核心区别：

| | Chunk（逻辑层） | Shard（物理层） |
|---|---|---|
| **所属层面** | Client / MetaServer | ChunkServer |
| **标识方式** | `chunk_id`（全局唯一） | `chunk_id` + `shard_index` |
| **数据内容** | 完整的用户数据 | 用户数据的一个 replica shard / data shard / code shard / local shard |
| **操作主体** | Client 通过 ChunkClient 操作 | ChunkServer 本地读写 |
| **一致性保证** | ChunkClient 保证所有分片在同一 offset 处一致 | 单个分片由 ChunkServer 串行写入保证顺序 |

> **术语约定**：本文后续章节中，ChunkServer 侧描述的读写、存储布局、崩溃恢复等操作均指**对单个分片**的操作；Client 侧描述的 Chunk 操作指对**逻辑 Chunk 整体**的操作（涉及所有分片的协调）。不需要区分特定冗余模式时，统一使用"分片"描述。

### 3.3 Chunk 状态机

逻辑 Chunk 在 MetaServer 侧维护一组**有限状态**，用于路由、修复与 GC。本节状态名统一为：**RW**（读写，全大写）、**Sealed**、**Unhealthy**、**DataLost**、**Deleted**（其余 PascalCase）。**不存在**名为「Corrupted」的独立逻辑 Chunk 状态；不可恢复情形统一为 **DataLost**。

#### 可用分片数 A 与不可用分片数 L

- **A**：当前**可用分片数**（仍可用于读路径或参与修复语义的分片计数；实现需与心跳、校验、路由一致）。
- **L**：**不可用分片数**。因**丢失**（不可达/缺失）与**损坏**（校验失败等）导致的不可用分片**合并计入 L**，不单独维护「损坏分片数」。
- 设该 Chunk 设计分片总数为 **max_shards**，且与 A、L 口径一致时：**A + L = max_shards**，即 **L = max_shards − A**。

#### 冗余策略参数 min_shards、max_shards

由冗余模式决定读/恢复所需最少分片数 **min_shards** 与总分片数 **max_shards**：

| 模式 | min_shards | max_shards | 说明 |
|------|------------|------------|------|
| **m 副本**（Replica） | **1** | **m** | 共 m 个 replica shard；至少 1 个可用即可读。 |
| **EC(m+n)**（m 个 data shard + n 个 code shard，共 **m+n** 分片） | **m** | **m+n** | 纠删解码至少需要 **m** 个分片；全量共 **m+n**。 |

（与 3.4 节 EC 记法一致时，可将 **EC(m+n)** 与「m 数据 + n 校验」对应。）

#### 由 A 判定健康类状态（核心规则）

记 **A** 为当前可用分片数，**min_shards**、**max_shards** 由上表根据该 Chunk 冗余模式得到：

1. 若 **A < min_shards** → **DataLost**（已无法按策略完成读/重建）。
2. 否则若 **min_shards ≤ A < max_shards** → **Unhealthy**（降级但在冗余可恢复范围内，应触发修复）。
3. 否则 **A = max_shards**：若 Chunk **已封存** → **Sealed**，否则 → **RW**。

**Deleted** 由删除、GC 等控制面流程从上述非终态转入（物理清理可异步，实现可再细分内部子状态，对外仍以 **Deleted** 为逻辑终态）。
...
| 逻辑 Chunk 状态 | 条件（L） |
|-----------------|-----------|
| **DataLost** | **L > max_shards − min_shards** |
| **Unhealthy** | **0 < L ≤ max_shards − min_shards** |
| **RW** / **Sealed**（健康） | **L = 0**（并区分是否已封存） |

文档叙述可以 **A 为主**（与运维直观一致），实现侧可只维护 **L** 或 **A** 之一并换算。

#### 状态含义摘要

| 状态 | 含义 |
|------|------|
| **RW** | 未封存、可追加写；**A = max_shards**（健康）。 |
| **Sealed** | 已封存、只读；**A = max_shards**（健康）。 |
| **Unhealthy** | **min_shards ≤ A < max_shards**；可修复。 |
| **DataLost** | **A < min_shards**；不可恢复。 |
| **Deleted** | 逻辑删除 / GC 终态。 |

#### 状态转换（含 Seal 与修复约束）

- **RW → Sealed**：Client Seal（写满、关闭、Non-Stop 切换 Chunk 等）。
- **RW / Sealed → Unhealthy**：**min_shards ≤ A < max_shards**（或等价 **0 < L ≤ max_shards − min_shards**）。
- **RW / Sealed → DataLost**：**A < min_shards**。
...
stateDiagram-v2
  ...
  Unhealthy --> Sealed: "repair, A = max_shards"
...
| 编码类型 | 说明 | 存储开销 | 容错能力 |
|----------|------|----------|----------|
| **Replica（副本）** | 完整复制 N 份（N 个 replica shard），通常 N=3 | 3x | 容忍任意 2 个分片丢失 |
| **EC（Erasure Coding）** | 如 EC(4,2)，4 个 data shard + 2 个 code shard | 1.5x | 容忍任意 2 个分片丢失 |
| **LRC（Local Reconstruction Code）** | EC 的改进，增加 local shard（本地校验分片），加速单分片修复 | 略高于 EC | 容忍多分片丢失，单分片修复更快 |
...
File "/data/log_20260322.dat"
  ...
  └── Chunk 3: chunk_id=1004, size=37MB, status=RW  ◄── 当前写入点
```
...
| 元数据类型 | 描述 |
|-----------|------|
| **ChunkInfo** | Chunk 的基本信息：chunk_id、状态（RW/Sealed/Unhealthy/DataLost/Deleted）、已写入长度、冗余编码类型、各分片所在 ChunkServer 列表 |
...
参考 GFS 的设计，MetaServer **不持久化**分片的位置信息，而是：

1. MetaServer 启动时，通过向所有 ChunkServer 轮询来重建分片位置信息
...
MetaServer 周期性收集 ChunkServer 上的分片信息。当某个 Chunk 的可用分片数 **A** 满足 **min_shards ≤ A < max_shards**（逻辑状态为 **Unhealthy**，见 3.3 节）或向 **DataLost** 发展时，按策略立即发起修复：
...
ChunkServer 是数据存储节点，负责 **Chunk 分片（Shard）** 的实际读写和本地数据管理。ChunkServer 不感知逻辑 Chunk 的全貌，它只管理和操作分配给自己的单个分片——无论该分片是副本分片（replica shard）、数据分片（data shard）、EC 校验分片（code shard）还是本地校验分片（local shard）。
...
### 5.2 分片的自包含布局（Self-Contained Shard Layout）

参考 Pangu 的自包含布局设计，每个**分片**的数据和元数据存储在同一个文件中（下称"分片文件"），一次 I/O 即可完成写入，减少写入延迟并延长 SSD 寿命。每个 ChunkServer 上的每个分片对应一个独立的分片文件。

```
┌──────────────────────────────────────────────────────┐
│               分片文件布局（Shard File Layout）           │
│                                                      │
│  ┌──────────────┐                                    │
...
**ChunkHeader（4KB 文件头）**：存储分片的全部元数据，包括 magic number、格式版本、所属 chunk_id、shard_index、已写入数据大小、分片状态、冗余编码信息、创建/修改/封存时间戳以及 Header 自身的 CRC32 校验。

**SectorFooter（每 4KB 尾部 24B）**：每个 4KB Sector 尾部嵌入校验信息，包括所属 chunk_id（防止 misdirected write）、sector 序号、上一笔/当前笔写入边界标记（prev_size/curr_size）以及本 Sector 的 CRC32。这种设计使得分片数据具备自校验能力，ChunkServer 可以独立从故障中恢复。

### 5.3 写入流程

ChunkServer 收到 ChunkClient 下发的写入请求后，对本地持有的**单个分片**执行以下流程：

```
收到 WriteRequest(chunk_id, offset, data, length)
  │
  ├── 1. 定位 ShardHandle（每个活跃分片在本地对应一个 Handle）
  │
  ├── 2. 校验 offset == 分片当前长度（committed_offset）
  │      ├── offset > committed_offset → 请求 pending，等待前序请求到来
  │      ├── offset < committed_offset → 拒绝（重复写入）
  │      └── offset == committed_offset → 执行写入
  │
  ├── 3. 串行写入磁盘（同一分片同一时刻最多一笔写入在执行）
  │      │
  │      ├── 计算涉及的 Sector 范围
...
- **串行写入**：每个分片的写入请求必须串行执行。前一笔数据写入成功后，才可以写入下一笔。这是追加写系统的核心约束
- **异步串行**：串行与同步无耦合。实际系统中采用异步串行写入——请求到达后进入有序队列，由独立线程按 offset 顺序执行
- **空洞处理**：如果请求的 offset 超过当前分片长度（请求乱序到达），请求进入 pending 队列等待前序写入完成

### 5.4 读取流程

ChunkServer 收到 ChunkClient 下发的读取请求后，对本地持有的**单个分片**执行以下流程：
...
收到 ReadRequest(chunk_id, offset, length)
  ...
  │      ├── 校验通过 → 提取用户数据
  │      └── 校验失败 → 标记该分片损坏，上报 MetaServer（由 MetaServer 更新可用分片数 **A** 并聚合逻辑 Chunk 状态）
  │
  └── 4. 返回数据给 Client
```
...
- Sealed 状态的分片可以直接读取全部数据，无需经过 ShardHandle

### 5.5 本地存储引擎

ChunkServer 的本地存储引擎负责管理磁盘上的分片文件，支持三种实现方式：

| 方式 | 说明 | 适用场景 |
|------|------|----------|
| **基于本地文件系统** | 使用 Linux 文件系统（如 Ext4）管理分片文件，每个分片对应一个文件 | 开发简单、兼容性好 |
...
- 利用分片自包含布局避免 page cache 和 journal 的开销
...
ChunkServer 崩溃重启后，对每个对应逻辑 Chunk 为 **RW**、尚未 **Sealed** 的**本地分片**执行恢复：
...
4. **截断与恢复**：将分片截断到最后一笔完整写入的位置，更新 Header

为加速崩溃恢复，ChunkServer 后台定时（默认每 5 秒）将每个分片内存中的 `committed_offset` 持久化到其 ChunkHeader 的 `committed_size` 字段。这意味着每个分片恢复时最多需要扫描 5 秒内写入的数据量。
...
- **后台扫描**：空闲时段周期性扫描 Sealed 状态分片的全部 Sector，检测静默数据损坏。发现损坏后标记该分片损坏，上报 MetaServer 触发修复（更新 **A**/**L** 与 Chunk 状态）
...
MetaServer 通过心跳回复向 ChunkServer 下达指令，如删除不再需要的分片、执行修复任务等。
...
ChunkClient::Read(chunk_id, offset, length)
  ...
  │      ├── Replica 模式: 选择一个最优副本分片读取
  │      └── EC 模式: 计算数据分散在哪些数据分片，向对应 ChunkServer 发起读取
  │
  ├── 3. 等待数据返回
  │      ├── 全部返回 → 解码后返回用户
...
**Backup Read（备份读）**：参考 Pangu 设计，当读请求超过预期延迟仍未返回时，Client 向其他副本分片或校验分片发起备份读。系统根据磁盘类型和 I/O 大小动态调整发起备份读的时机，并限制备份读的数量以控制系统负载。

#### 6.1.3 封存逻辑（Seal）

Seal 操作将 Chunk 从 **RW** 转为 **Sealed**，阻止后续写入。触发 Seal 的场景：
...
| Close | 关闭文件，Seal 当前 **RW** 状态的 Chunk |
| Delete | 删除文件（延迟回收） |
| Rename | 原子重命名 |
...
| 故障类型 | 检测方式 | 处理策略 |
|----------|----------|----------|
| 节点宕机 | MetaServer 心跳超时 | 自动触发受影响 Chunk 的分片补充/EC 修复 |
| 磁盘故障 | ChunkServer 本地检测 + 慢 I/O 统计 | 将磁盘从服务列表摘除，上报 MetaServer |
| 静默数据损坏 | CRC 校验（读取时 + 后台扫描） | 标记分片损坏，MetaServer 更新 **A**/**L** 并调度修复；Chunk 可能进入 **Unhealthy** / **DataLost** |
| 网络分区 | 心跳超时 | 等待恢复后重新汇报分片列表 |

### 8.3 Client 故障处理

- **写入超时重试**：ChunkClient 内置重试机制，写入失败后自动 Seal 并切换新 Chunk
- **读取 Backup Read**：读取超时后自动向其他副本分片或校验分片发起备份读
...
| **数据完整性** | CRC 校验失败次数、处于 **Unhealthy**/**DataLost** 的 Chunk 数量 |
| **磁盘健康** | 慢 I/O 次数、I/O 错误次数、磁盘利用率 |
| **Chunk 统计** | 总 Chunk 数、RW/Sealed/Unhealthy/DataLost/Deleted 各状态 Chunk 数 |
